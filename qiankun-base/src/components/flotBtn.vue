 ...
 * @FilePath: /qiankun/qiankun-base/src/components/flotBtn.vue
-->
...
<template>
  <div ref="page" :class="{ 'floating-window': true, zIndexTop: moveFlags }" @mouseup.stop.prevent="moveEnd">
    ...
    >
      ...
      <i :style="{ fontSize: `${form.fontSize}px` }" class="el-icon-user icon"></i>
...
export default {
  ...
  created() {
    document.addEventListener("mouseup", (e) => {
      if (e.clientY > window.innerHeight || e.clientY < 60 || e.clientX < 0 || e.clientX > window.innerWidth) {
        this.moveEnd();
      }
    });
  },
  ...
  methods: {
    ...
    moveStart(e) {
      ...
      document.onmousemove = async (e) => {
        ...
        this.yPum = (this.ny > moveMaxHeight ? moveMaxHeight : this.ny) - this.dy;
        await this.$nextTick();
        this.lastMoveIndex++;
